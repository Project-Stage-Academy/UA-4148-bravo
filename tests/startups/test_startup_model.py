...
class StartupModelCleanTests(BaseAPITestCase):
    """
    Tests for Startup.clean(): social_links validation, required fields, and bounds.
    """

    def test_valid_clean_should_pass(self):
        """
        All links valid & allowed → should not raise ValidationError.
        """
        startup = Startup(
            user=self.user,
    ...
    def test_invalid_social_links_clean_should_raise(self):
        """
        Unsupported platforms or invalid domains should raise ValidationError.
        """
        startup = Startup(
            user=self.user,
