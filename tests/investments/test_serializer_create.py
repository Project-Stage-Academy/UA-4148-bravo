...
class BaseSubscriptionSerializerTest(BaseAPITestCase):
...
class SubscriptionSerializerValidDataTests(BaseSubscriptionSerializerTest):
    ...
    def test_rounding_of_investment_share(self):
        ...
        recalculate_investment_shares(self.project)
        subscription.refresh_from_db()
        expected_share = (Decimal("333.33") / self.project.funding_goal * 100).quantize(
    ...
    def test_cumulative_rounding_errors(self):
        ...
        recalculate_investment_shares(self.project)
        total_share = sum(sub.refresh_from_db() or sub.investment_share for sub in subscriptions)
        self.assertLessEqual(total_share, Decimal("100.00"))
        self.assertGreaterEqual(total_share, Decimal("99.99"))


class SubscriptionSerializerAmountValidationTests(BaseSubscriptionSerializerTest):
...
class SubscriptionSerializerInvestmentConstraintsTests(BaseSubscriptionSerializerTest):
    def test_self_investment_rejected(self):
        ...
        self.assertTrue(
            any("cannot invest in their own project" in str(msg).lower() for msg in error_messages),
    ...
    def test_exceeds_funding_goal(self):
        ...
        data = self.get_subscription_data(self.investor2, self.project, amount2)
