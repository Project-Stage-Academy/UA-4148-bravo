...
from investors.models import RecentlyViewedStartup
...
class RecentlyViewedStartupTests(APITestCase):
    ...
    def test_record_viewed_startup(self):
        ...
        self.assertTrue(
            RecentlyViewedStartup.objects.filter(investor=self.investor, startup=self.startup).exists()
        )

    def test_list_recently_viewed(self):
        """Test retrieving list of recently viewed startups"""
        RecentlyViewedStartup.objects.create(investor=self.investor, startup=self.startup)
        url = reverse("recently-viewed-list")
    ...
    def test_clear_history(self):
        ...
        self.assertFalse(
            RecentlyViewedStartup.objects.filter(investor=self.investor).exists()
        )
