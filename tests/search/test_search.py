...
class SearchTests(APITestCase):
    def setUp(self):
        ...
        # Ensure visibility flags are set (in case serializers or views filter by them)
        changed = False
        ...
        self.project = Project.objects.create(
            ...
            title="Health AI",  # change to 'name' if the model uses a different field
            description="AI project for healthcare",
            status="active",    # or Project.Status.ACTIVE if using choices
    ...
    def test_startup_search_mocked(self):
        with mock.patch.object(sv.StartupDocument, "search") as search_fn:
            ...
            qs = Startup.objects.filter(pk=self.startup.pk)  # in case to_queryset() is used
            self._chainable_search(search_obj, [hit], qs=qs)

            url = reverse("startup-search")
            resp = self.client.get(url, {"q": "health"})  # longer than 2 characters
    ...
    def test_project_search_mocked(self):
        with mock.patch.object(sv.ProjectDocument, "search") as search_fn:
            ...
            self.assertEqual(resp.status_code, 200, resp.data)
            self.assertEqual(len(resp.data), 1, resp.data)
            self.assertEqual(resp.data[0]["title"], "Health AI")  # change key if serializer uses a different field
